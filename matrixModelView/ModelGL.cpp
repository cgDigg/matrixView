                     ...
                     cameraDistance(CAMERA_DISTANCE), windowSizeChanged(false)
{
	...
	m_left = m_bottom = m_front = -10;
	m_right = m_top = m_back = 10;
	m_front = -100;
	m_back = 100;
}
...
void ModelGL::setViewportSub(int x, int y, int width, int height, float nearPlane, float farPlane)
{
	...
	setOrthoFrustum(m_left, m_right, m_bottom, m_top, m_front, m_back);
    // copy projection matrix to OpenGL
    glMatrixMode(GL_PROJECTION);
    glLoadMatrixf(matrixProjection.getTranspose());
    glMatrixMode(GL_MODELVIEW);
...
void ModelGL::drawSub2()
{
    ...
    glTranslatef(cameraPosition[0], cameraPosition[1], -cameraDistance);
...
{
    float tangent = 1/*tanf(fovY/2 * DEG2RAD)*/;
	aspectRatio = 1;
...
///////////////////////////////////////////////////////////////////////////////
// move the model for subWin1
///////////////////////////////////////////////////////////////////////////////
void ModelGL::moveModel(int x, int y)
{
	modelPosition[0] += x;
	modelPosition[1] -= y;
	cameraPosition[0] += x;
...
void ModelGL::rotateModel(int x, int y)
{
	float dx = x / (m_right - m_left);
	float dy = y / (m_top - m_bottom);
	float n[3] = { -dy, -dx, 0 }; // Ðý×ªÏòÁ¿
	float beta1 = -2 * PI*sqrtf(dx*dx + dy*dy);
	float alpha = acosf(-dx / sqrtf(dx*dx + dy*dy));
	float beta = acosf(-dy / sqrtf(dx*dx + dy*dy));
	modelAngle[0] += alpha;
	modelAngle[1] += beta;
...
{
	float increase = (m_right - m_left)*0.1f;

	double left = m_left;
	double right = m_right;
